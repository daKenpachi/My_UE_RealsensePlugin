...
// Sets default values
URealsenseInfraredStereoComponent::URealsenseInfraredStereoComponent()
{
...
bool URealsenseInfraredStereoComponent::ReceiveFrame()
{
	try {
		...
		TextureLeft->UpdateTextureRegions(0, 1, TextureUpdateRegion, static_cast<uint32>(WidthLeft * frameLeft.get_bytes_per_pixel()),
			frameLeft.get_bytes_per_pixel(), (uint8*) frameLeft.get_data(), texCleanUpFP);
		TextureRight->UpdateTextureRegions(0, 1, TextureUpdateRegion, static_cast<uint32>(WidthLeft * frameRight.get_bytes_per_pixel()),
...
UTexture2D * URealsenseInfraredStereoComponent::getTextureLeft()
{
	return TextureLeft;
}

UTexture2D * URealsenseInfraredStereoComponent::getTextureRight()
{
	return TextureRight;
}

// Called when the game starts or when spawned
void URealsenseInfraredStereoComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	try {
		...
		TextureUpdateRegion = new FUpdateTextureRegion2D(0, 0, 0, 0, WidthLeft, HeightLeft);
	}
	catch (std::exception e) {
...
void URealsenseInfraredStereoComponent::EndPlay()
{
	if (pipeline) {
		pipeline->stop();
	}
