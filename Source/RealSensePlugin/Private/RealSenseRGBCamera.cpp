...
ARealSenseRGBCamera::ARealSenseRGBCamera()
{
}
...
void ARealSenseRGBCamera::CreateUpdateableTextures()
{
...
UTexture2D*  ARealSenseRGBCamera::ReceiveRGBFrame()
{
	if (cameraWorks) {
		TextureFromVideo->UpdateResource();
		if (!receiveFrames()) {
...
UTexture2D * ARealSenseRGBCamera::ReceiveDepthFrame()
{
	if (!Enable_d) {
...
void ARealSenseRGBCamera::BeginPlay()
{
	Super::BeginPlay();
...
bool ARealSenseRGBCamera::receiveFrames()
{
	try {
		rs2::frameset frames = pipeline->wait_for_frames();
		rs2::video_frame colorFrame = frames.get_color_frame();
...
// Called every frame
void ARealSenseRGBCamera::Tick(float DeltaTime)
{
	//TextureFromVideo->UpdateResource();
